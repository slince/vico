...
/**
 * 统一入口：根据 config.server.agent_engine 选择增强引擎或 Legacy pipeline。
 * 增强引擎集成 4 个 Bridge + 3 个 Processor，提供审计日志、Token 统计、增强消息持久化。
 * 前端和 API 路由层无需感知引擎切换。
 * 增强引擎失败时自动回退到 Legacy pipeline。
 */
export async function runChatPipeline(
  ...
  if (engine === 'mastra') {
    try {
      ...
      return await createMastraAgent(ctx, message);
    } catch (err) {
      console.error('[EnhancedEngine] Error, falling back to legacy pipeline:', err);
    }
  }
