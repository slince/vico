...
export { createSkillExecuteTool } from './skill-execute.js';

/**
 * 创建 Skill 模块的全部工具（load / search / read / execute）。
 * @param skills - skills 实例
 * @returns 包含四个 Skill 工具的数组
 */
export function createSkillTools(skills: Skill[]): Tool[] {
  ...
  return [
    ...
    createSkillExecuteTool({ getSkills: () => skills }),
  ];
}
