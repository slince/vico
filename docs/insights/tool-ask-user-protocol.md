...
→ ToolExecutor 逐 call 把 decision 注入 ToolCallContext.approval → ask_user.execute 读 answer 解析
```

关键设计：
- 答案复用 `tool-approval-response` 的 `reason` 字段承载（AI SDK 审批协议里唯一可带文本的字段），结构化编码为 JSON 对象，后端 `parseAskUserAnswers` 严格解析 + zod 校验。
- 注入点下沉到工具层：`ToolExecutor.execute` 逐 call 把 `TurnContext.decisions.get(call.id)` 写入 `ToolCallContext.approval`；`ask_user.execute` 从 `ctx.approval.answer` 读取并解析。loop-agent 主流程不再对 ask_user 特殊分支，澄清类工具与普通审批工具统一走 `approvedCalls` 正常 `execute`。
...
- 后端：`packages/core/src/tool/builtin/basic/ask-user-tool.ts`（schema + `parseAskUserAnswers` + execute 读 ctx.approval）
